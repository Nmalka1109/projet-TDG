...
void ms(graphe &a1)
{
    ...
    do
    {
        std::cout<<"saisissez le poids du sommet a changer"<<std::endl;
        std::cin>>m_sommet;
...
void ma(graphe &a1)
{
    ...
    std::cin>>j;
    std::cout<<"saisissez le poids de l'arete a changer"<<std::endl;
    std::cin>>m_arete;
...
void simulation(graphe &a1)
{
    ...
    ///on réinitialise le poids du sommet à son poids d'origine
    for(int i=0; i<a1.m_ordre; i++)
    {
        a1.m_som[i].p_sim=a1.m_som[i].poids;
    }
        ...
        for(int w=0; w<t; w++)
        {
            ...
            for(int x=0; x<a1.m_ordre; x++)
            {
                ...
                for(int i=0; i<a1.m_ordre; i++)
                {
                    if(adj[i][x]==1)
                    {
                        for(int h=0; h<a1.m_ar.size(); h++)
                        {
                            if(a1.m_ar[h].m_s1==i && a1.m_ar[h].m_s2==x)
                            {
                                k+=a1.m_ar[h].m_pa*a1.m_som[i].p_sim;
                    ...
                    ///somme des poids de aretes et des sommets de tous les prédecesseurs (nourriture disponible)
                    // k+=a1.m_ar[pre[x][i]].m_pa*a1.m_som[pre[x][i]].p_sim;
                }
                ...
                for(int y=0; y<a1.m_ordre; y++)
                {
                    ...
                    if(adj[x][y]==1)
                    {
                        for(int d=0; d<a1.m_ordre; d++)
                        {
                            if(a1.m_ar[d].m_s1==x && a1.m_ar[d].m_s2==y)
                            {
                                greg+=a1.m_ar[d].m_pa*a1.m_som[y].p_sim;
                            }
                        }
                        //cmp++;
                        //nat.push_back(y);
                    }
                    ///développement de la population - le nombre de successeurs (tous les prédateurs)
                    a1.m_som[x].p_sim=a1.m_som[x].rythme*a1.m_som[x].p_sim*(1-a1.m_som[x].p_sim/k)-greg;
                }
                //suc.push_back(nat);
                for(int i=0; i<cmp; i++)
                {
                    //greg+=a1.m_ar[suc[x][i]].m_pa*a1.m_som[suc[x][i]].p_sim;
                }

                if(a1.m_som[x].p_sim<0)
                {
                    a1.m_som[x].p_sim=0;
                }
            ...
            for(int g=0; g<test.size(); g++)
            {
                /*for(int h=0;h<a1.m_ordre;h++)
                {
                    std::cout << g<< " "<<h<<"  "<<pre[g][h]<<std::endl;
                }*/
                //std::cout << g<< " "<<test[g]<<std::endl;
            }
...
int affichage(graphe &a1, int fond)
{
    ...
    ///afficher un fond d'écran en fonction du choix de l'utilisateur
    if(fond==1)
    {
    ...
    for(int i=0; i<a1.m_ordre; i++)
    {
        if( a1.m_som[i].actif == 1)
        {
            ...
            ///affichage du numéro de sommet
            textprintf_centre_ex(page, font,a1.m_som[i].getx()+15, a1.m_som[i].gety()+110,makecol(255,0,0),-1,"%d", i);
            ///affichage du poids du sommet
            if(verif<0)
            {
                textprintf_centre_ex(page, font,a1.m_som[i].getx()+100, a1.m_som[i].gety()+110,makecol(255,0,0),-1,"%d", a1.m_som[i].p_sim);
        ...
        if (a1.m_som[i].actif==2)
        {
            circlefill(page,a1.m_som[i].getx()+60, a1.m_som[i].gety()+60,65,makecol(255,255,255));
        }
    }

    for(int j=0; j<a1.m_ar.size(); j++)
    {
        if(a1.m_som[a1.m_ar[j].m_s1].actif!=0 && a1.m_som[a1.m_ar[j].m_s2].actif!=0 && a1.m_som[a1.m_ar[j].m_s2].actif!=3 && a1.m_som[a1.m_ar[j].m_s1].actif!=3)
        {
    ...
    if(e>0)
    {
        for (int i =0; i<a1.m_som.size(); i++)
        {
            if(a1.m_som[i].actif==2 || a1.m_som[i].actif==3)
            {
                a1.m_som[i].actif=1;
            }
        }
...
void Duree(graphe &a1)
{
    ...
    if(mouse_x>580+a1.gettemps() && mouse_x<620+a1.gettemps() && mouse_y > 640 && mouse_y < 670 && mouse_b==1)
    {
        while(mouse_b==1)
        {
            ...
            textprintf_centre_ex(buffer, font,680,680,makecol(255,255,255),12,"Année : %d",t);
    ...
    else
    {
        ...
        t=a1.gettemps()/10;
        textprintf_centre_ex(page, font,680,680,makecol(255,255,255),12,"Année : %d",t);
    }
}

void connexite(graphe &a1, int a)
{
    vector<int>w;
    w=a1.cxt(a);
    textprintf_centre_ex(page,font,43,450,makecol(255,255,255),-1,"CFC : %d",w.size());
}
}
